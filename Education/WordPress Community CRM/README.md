...
| --- | --- | --- | --- |
| [`community-crm`](./community-crm) | **WordPress Community CRM** | 1.1.5 | Full Site Editing (block) theme — the front end of the CRM. Its front page doubles as the sign-in screen: one card holding an illustrated hero, a WordPress.org sign-in form and a request-access panel, framed by a thin header bar and a footer fixed to the bottom of the window. Built on WordPress Design System tokens (brand blue `#3858e9`, 13px system type, 4px spacing base, 2px control radius) mapped into `theme.json`, so the whole design is editable in the Site Editor. Ships a matching skin for `wp-login.php` so both sign-in routes look like the same product, plus templates for pages, posts, archives, search and 404, and patterns for the hero and the sign-in card. No build step. |
...
| --- | --- | --- | --- | --- |
| `community-crm` (theme) | WordPress 6.5 | 6.8 | 7.4 | — |
