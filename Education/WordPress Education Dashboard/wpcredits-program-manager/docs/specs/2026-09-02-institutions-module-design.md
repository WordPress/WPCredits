...
24. **The 38 Confirmed institutions without recorded consent all hold a signed Collaboration Agreement** in Drive folder `1DqALNg4cxF-hdANB8s1iRtqGn2hok5Zr`. That is the legal basis for provisioning and emailing them. They are never forced back through the gate: a program manager records an "agreement on file" state with a Drive link, and the bulk provisioning proceeds once every Confirmed institution has one.
...
### Students (`tbla8GZg5x6NY7aWt`), 800 rows, 28 fields
...
**Mail**, both through `WPCPM_Mail`. `send_to( $email, $context, $build, $locale )` is added beside `send()` as its own commit with `bin/test-mail.php` cases, because it touches the single exit for all plugin mail. `institution-applied` to the applicant carries the reference, what happens next, **the routed manager's Calendly link** from `WPCPM_Countries::routing()`, and the address-verification link (a `wp_hash()`-signed single-use token that stamps `_wpcpm_app_verified`). `institution-application` to the managers with a link to the queue row, for submissions that pass the checks; held rows wait silently.
...
`includes/class-wpcpm-private-files.php`. Base `wp_upload_dir()['basedir'] . '/wpcpm-private/'`, `wp_mkdir_p()`, `index.php`, an `.htaccess` with `Require all denied` and `Deny from all`. Path `agreements/<YYYY>/<32 hex from random_bytes(16)>.pdf`, mode 0640. **The plugin never prints the file's URL anywhere**; the only route to the bytes is `handle_download()`. `path( $relative )` resolves through `realpath()` and refuses anything outside the base.
...
`WPCPM_Field_Value::clean()` gains `date` (empty is `null`, which clears; otherwise pattern and `checkdate()`) and `select` (membership in a server-held list, never a pass-through). Cross-field rules run over the accepted cells with the record's current values underneath; `max_days` refuses 2036.
...
These need a human decision. Everything else in this document is decided.

1. **Does "graduated" include Pending graduation?** The Krakow reference counted its two as graduates (10); the Students table says 8 plus 2. The design prints both lines until answered; a yes folds `pending` into `graduated`.
2. **Who may send the consent request to the 289 students who have never been asked?** Institution members and program managers as designed, or managers only. Graduates who never log in again will never answer, and the first reports will look thinner than the hand-written one either way.
3. **Who reads the mentor request queue, and after how many days is a "waiting for a mentor" item overdue?** Phase 5 does not ship without a named person.
4. **Will the base owner add `Paused` to view `viwzSJspvACLnhXom`, and have the creation automation copy `WP Profile` and `Your field of study` to the reports row?** Both are automation changes outside decision 17's procedure and need an owner. Until the first is done, an institution cannot graduate a Paused student.
5. **Can the "WordPress Credits - Student Registration Form" be closed, or refuse an existing address, for institutions that import?** A second submission creates a duplicate row the plugin cannot prevent. What does the form collect that an imported student still has to provide, and through what channel?
6. **May an institution edit a student's email address**, and should the write also change the WordPress account? Out of the allowlist for two independent reasons; putting it back needs both answered.
7. **Should a Pending graduation student be able to log hours?** The hours box lives inside "My course", absent for any status without a course URL, so the nine newly tracked students cannot.
8. **Which fields are "details related to the project" for a joint project, and is the student's name among them?** `PROJECT_FIELDS` precedes the entity.
...
10. ~~What does Atomic do with a direct request to `/wp-content/uploads/wpcpm-private/`?~~ **Answered, 2 September 2026, by probing the host.** Directory requests under `wp-content/uploads` return 403 from nginx with no listing, and a request for a file that does not exist returns 404. But a file that *does* exist there is served by nginx as a static asset to anyone who knows its name, and the host offers no directory outside the web root. So entropy in the filename is a mitigation, not the control: the signed agreement is served only through the plugin's own download handler, which checks the capability and the institution before streaming the file, and the stored file carries a non-guessable name and a `.htaccess`-independent deny where the host honours one. Never a direct URL. This is no longer open.
11. **The agreement's administration**: who is the wording owner of the Doc and can its sharing be tightened to viewer; does the program countersign, and who signs for the Foundation; which other language versions exist, and are the two bespoke agreements in English; may an institution state the printed name (designed: yes, editable and recorded beside the Airtable name)?
12. **Where does a mentor-plus-member account land on login**, and should `ACT_MANAGE_MEMBERS` be exempt from the gate so the contact can invite the signatory before the agreement is settled? Decision 6 says "exactly one thing" and the design follows it; the manager's add-account is the workaround, and a yes is one entry in `ungated()`.
13. **Retention and ceilings to match the published privacy notice**: 30 / 365 / never for applications; 30 days for withdrawn and returned agreement files and never for accepted ones; 7 and 30 days for import batches; 14-day invitations, 10 pending per institution, 5 sends per member per day; 10 MB and 5 uploads a day for agreements. All placeholders.
14. **The page title** for slug `institution-dashboard`, chosen deliberately under the full-product-name rule; and whether the institution should see `Accessibility needs` at all (excluded everywhere here) and individual course grades beyond the single-student view (the line drawn here).
15. **Should the accepted PDF also be pushed onto the Airtable record**, and should Airtable carry a read-only "Site accounts" count? Both are one announced field if wanted; neither is in this design.
16. **Should a program manager be able to import, generate or accept on behalf of an institution at an unsettled stage as designed** (yes, logged as `manager`), or should the gate apply to managers for the semester report specifically? The design says the manager is the person the gate is waiting for.
