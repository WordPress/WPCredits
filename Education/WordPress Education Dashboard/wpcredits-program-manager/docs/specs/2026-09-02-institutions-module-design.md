...
41 of the 42 Confirmed carry a `Contact Email`; one does not, and `wp_insert_user()` cannot make an account without one. No duplicate contact emails and no duplicate names among the Confirmed. Ten names end in a trailing space. Two records have no `Name` at all. Tutors are linked on 3 records. **No field records whether an agreement exists, what kind, when it was accepted or where it is**; `Agreement Sent` is a stage, not a document.
...
758 addresses exist in both tables. On every joined pair the two institution links **agree**; the "10 institutions where the two link counts disagree" (one institution 16 against 27, another 66 against 61, a third 29 against 22, and so on) are duplicate and orphan rows, not students filed under different schools. Students rows with no reports row: **31** (Not moving forward 15, empty 7, Graduate 6, In Sensei 2, SPAM 1). Reports rows with no Students row: **19** (In Sensei 7, Graduate 6, Not moving forward 4, empty 1, SPAM 1), seven of which have site accounts today. Status disagreements on joined rows: **10**, four of them rows at one institution reading `Paused` on Students and `Not moving forward` on Students Reports.
...
**One institution is the reference report exactly**: 15 rows, all 2026 H1, Graduate 8, Pending graduation 2, Not moving forward 5. The hand-written report says "15 signed up, 10 graduated". So *signed up* counts everyone including the five who never started, and *graduated* counted the two at Pending graduation. The first is adopted; the second is open question 1.

A Polish institution calls February to June the *summer* semester; a US institution calls it *spring*. No label in this module ever carries a season word.
...
`ACTION_ASK` (`report-consent` mail): one message per student in the cohort with a candidate answer and no permission answer, through `WPCPM_Mail::send()`, never twice inside 30 days, stopping at 25 per press with the rest queued. **Program managers only** (open question 2): the button is on the manager screen, the handler requires `CAP_MANAGE` on top of the policy, and the institution's own report shows the withheld count and a sentence saying a manager can send the request, never the send itself.
