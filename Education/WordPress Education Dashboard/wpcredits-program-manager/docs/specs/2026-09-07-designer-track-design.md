...
**Project:** the eight practical lessons first, each a lead naming the lesson exactly as Learn does, with its fields under it:
...
Then the project lesson, where Learn places it: `Main Contribution Team` under the lead "Define and begin developing your contribution project", so the team list, `Contribution Project Summary` directly under it and `Optional: Additional Contribution Project Summary` (the Developer Track's second project, unpaired) all read as that lesson's questions (the product owner, 8 September 2026, three times: the project questions moved under the team, the heading moved over the team, and the block moved after the practical lessons, directly above the reflection posts). Then the three reflection posts (Choosing Your Team and Project; Your First Contribution; Halfway Check-In) under "Your reflection posts", then the alumni program section exactly as the Developer Track shows it (the product owner, 8 September 2026): `Slack/GitHub/Blog WordPress Community meetings/discussions` opens it under the subgroup heading "Alumni Program" (the lesson's short name, not its full Learn title), followed by the Developer Track's three alumni answers word for word - `Contributing beyond WP Credits`, `Alumni program: personal email`, `Alumni program: mentoring opt-in`; then `WP event participation URL` under the lead "Participate at a WordPress Event (online or in person)", so the alumni section is closed on both sides. Forty-nine fields in all.
