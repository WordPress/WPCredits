...
**Project:** `Main Contribution Team` under the lead "Define and begin developing your contribution project", so the team list, `Contribution Project Summary` directly under it and `Optional: Additional Contribution Project Summary` (the Developer Track's second project, unpaired) all read as that lesson's questions, before the lessons (the product owner, 8 September 2026, twice: the project questions moved under the team on the morning's request, the heading moved over the team on the afternoon's); then the eight practical lessons, each a lead naming the lesson exactly as Learn does, with its fields under it:
