...
**Blank.** A status, a key and a label, and an empty form. "New track" above the list asks for exactly those three, the things two tracks can never share, and the hue starts as the first one no live track uses, as a duplicate's does; the rest of the track's properties and every question are edited on the track's page it opens.
...
**The track list.** One row per track: name, status, key chip, source (built-in or definition), state (draft, published, "Unpublished changes", or, for a built-in track still running from its PHP, "Live, from its hand-written form" with a line saying its definition is not published yet and what publishing it does), how many students are on it now, its Learn course, who published it last and when, and the Airtable steps still outstanding. New track (blank, or from a Learn course link), Duplicate, Edit, Preview, History, Delete on a track that was never published (decision 25), and for a built-in track the equivalence line, the switch, and "Publish definition" in place of Publish (decision 29).
