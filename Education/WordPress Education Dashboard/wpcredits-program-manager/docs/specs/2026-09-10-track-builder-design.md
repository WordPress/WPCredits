...
**Status:** approved by the product owner on 10 September 2026, after design sessions on 7, 8 and 10 September; not yet implemented. Phase T1 is planned in `docs/plans/2026-09-10-track-builder-t1.md`.
...
| **T2** | `builtin_fields()`, `seed_tracks` and the four seeds, the equivalence check and the switch, and the Track Builder screen: the track list, track properties, duplication, the read-only preflight, verify, the checklist, and publishing of tracks whose columns all exist. | Reads only. A person adds the choices and changes the automations. |
...
| **T5** | The PHP removed, on the product owner's word. | Nothing |
