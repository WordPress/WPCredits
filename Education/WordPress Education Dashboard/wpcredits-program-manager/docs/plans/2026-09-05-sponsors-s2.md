...
ck( 'a shared offer shows its own link in the form and no codes box', array( false !== strpos( $html_b, 'value="https://cirrus-example.example/checkout?code=WPCREDITS"' ), strpos( $html_b, 'name="wpcpm_codes"' ) ), array( true, false ) );
...
5. Restore `sponsor_notify` to '', the index row's manager and the record's `Person of contact` (`recSEED000000902`). Leave the TEST offer live with its remaining codes: it is the demonstration.
