...
**Tech Stack:** WordPress 6.5 and PHP 7.4 as floors, WordPress coding standards, the plugin's standalone `bin/test-*.php` suites, `bin/build-docs.php` for the guides, no build step, no new script: the plugin's own `data-wpcpm-once`, `data-wpcpm-busy` and `data-wpcpm-confirm` attributes do what the existing forms' do. **Amended on 23 September 2026 (the deep check of 1.109.1, SESSIONS-6, as its refuter asked):** the reader of all three is the plugin's own `assets/js/forms.js`, not the theme; `data-wpcpm-confirm` is read by its `confirmFirst()` and `bindConfirm()` (lines 407 and 420 to 421), which 1.110.0 added after the mark was found carried and read by nothing.
